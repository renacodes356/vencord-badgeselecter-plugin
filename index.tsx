...
import { Menu, Toasts, FluxDispatcher } from "@webpack/common";
...
function getSignature(userId: string): string {
    const own = userBadgesMap.get(userId);
    const removed = removedBadgesMap.get(userId);
    const ownPart = own ? own.map(b => b.id).join(",") : "";
    const removedPart = removed ? Array.from(removed).sort().join(",") : "";
    return `${ownPart}|${removedPart}`;
}
...
function notifyProfileChanged(userId: string) {
    invalidateCache(userId);
    try {
        UserProfileStore.emitChange?.();
    } catch (err) {
        console.error("[BadgeSelector] failed to emit UserProfileStore change", err);
    }
    try {
...
function toggleBadge(userId: string, badge: any, hasActualBadge: boolean) {
    ...
    if (hasActualBadge) {
        if (isHidden) {
            ...
            Toasts.show({ message: `Restored ${badge.title}`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
        } else {
            hideRealBadge(userId, badge.id);
            Toasts.show({ message: `Removed ${badge.title}`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
        }
    } else {
        if (hasUserBadge) {
            ...
            Toasts.show({ message: `Added ${badge.title}`, type: Toasts.Type.SUCCESS, id: Toasts.genId() });
        }
    }
}
...
function giveAllBadges(userId: string) {
    ...
    Toasts.show({ message: "Added all badges", type: Toasts.Type.SUCCESS, id: Toasts.genId() });
}
...
export default definePlugin({
    name: "BadgeSelector",
    description: "Customize user badges - add, remove, or hide any Discord badge (visual only)",
    authors: [
    ...
    async start() {
        ...
        UserProfileStore.getUserProfile = function (userId: string, ...rest: any[]) {
            const userProfile = originalGetUserProfile.call(this, userId, ...rest);
            if (!userProfile) return userProfile;

            const hasCustomizations = userBadgesMap.has(userId) || removedBadgesMap.has(userId);
            if (!hasCustomizations) return userProfile;

            try {
                const signature = getSignature(userId);
                const cached = profileCache.get(userId);

                if (cached && cached.sourceProfile === userProfile && cached.signature === signature) {
                    return cached.result;
                }

                let newBadges = Array.isArray(userProfile.badges) ? Array.from(userProfile.badges) : [];
                ...
                const result = new Proxy(userProfile, {
                    get(target, prop) {
                        if (prop === "badges") return newBadges;
                        return Reflect.get(target, prop);
                    }
                });
                profileCache.set(userId, { sourceProfile: userProfile, signature, result });
                return result;
            } catch (err) {
                console.error("[BadgeSelector] failed to build custom badge list, falling back to original", err);
                return userProfile;
            }
        };
    },
                ...
                >
                    ...
                    <Menu.MenuItem
                        ...
                        action={() => {
                            ...
                            notifyProfileChanged(user.id);

                            Toasts.show({ message: "Removed all badges", type: Toasts.Type.SUCCESS, id: Toasts.genId() });
                        }}
                    />
                    <Menu.MenuItem
                        ...
                        action={() => {
                            ...
                            Toasts.show({ message: "Reset badges to original state", type: Toasts.Type.SUCCESS, id: Toasts.genId() });
                        }}
                    />
                </Menu.MenuItem>
            );
